...
# 定义颜色分量的名称
color_features = ['蓝色', '绿色', '红色', '色调', '饱和度']
...
# 显示每个物质的最佳拟合图像，并输出回归方程
for fit in best_fits:
    print(f"物质 {fit['substance']} ({fit['feature']}) 的最佳回归方程: y = {fit['equation']} (R²={fit['r2']:.4f})")

    plt.figure()
    plt.scatter(fit['y'], fit['X'], color='blue', label='实际值')
    plt.plot(fit['y_pred'], fit['X_fit'], color='red', label='拟合曲线')
    plt.title(f'物质 {fit["substance"]} ({fit["feature"]}): 实际值与预测值 (R²={fit["r2"]:.4f})')
    plt.xlabel('浓度')
    plt.ylabel(f'{fit["feature"]} 通道特征值')
    plt.grid(True)
    plt.legend()
    plt.show()
